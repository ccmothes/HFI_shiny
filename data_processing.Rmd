...
with_progress({
  ...
  results <- map2_dfr(aligned_rasters, years, function(raster, year) {
    p(sprintf("Processing year %s", year))
    
    # For each country (sequential within each year)
    map_dfr(seq_len(nrow(countries)), function(country_idx) {
      # Mask raster to country
      country_geom <- countries[country_idx]
      masked_raster <- mask(raster, country_geom)
      
      # Classify and calculate proportions
      r_classified <- classify(masked_raster, rcl = rcl)
      freq_table <- freq(r_classified)
      
      # Skip if no data
      if(nrow(freq_table) == 0) return(NULL)
      
      freq_table %>%
        mutate(
          proportion = count / sum(count),
          year = year,
          class = c("Low", "Medium", "High")[value],
          country = countries$name[country_idx]
        )
    })
  })
})
