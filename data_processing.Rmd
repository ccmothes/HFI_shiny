...
rasters <- list.files("data/final_data/", pattern = ".tif$", full.names = TRUE)

# annual and change rasters

change <- terra::rast(rasters[str_detect(rasters, "diff_")])

annual <- map(rasters[str_detect(rasters, "mosaic")], terra::rast)
```

Get file names to use later on

```{r}
annual_names <- map(rasters[str_detect(rasters, "mosaic")], ~str_extract(.x, "(?<=_)\\d{4}(?=_)"))
```
...
Read in rasters
```{r}
rasters <- terra::rast("data/cleaned_rasters_2025-12-05.tif")
```
...
```{r}
rasters <- rast("data/cleaned_rasters_2025-12-05.tif")
```
...
## Reclassify Change Map
```{r}
change_update <- change

change_update[change_update >= -10 & change_update <= 10] <- NA

```

Check new values
```{r}
change_vals <- terra::freq(change_update)
```

Still inclusive of -10 and 10...keeping for now
Inspect values
```{r}
plot <- ggplot(change_vals) +
  geom_col(aes(x = value, y = count))
...
```{r}
change_update_prj <- terra::project(change_update, "EPSG:3857")
```


```{r}
writeRaster(change_update, "data/extreme_diff_2022to2024_vs_1999to2000.tif")

# save projected raster too
writeRaster(change_update_prj, "data/extreme_prj_diff_2022to2024_vs_1999to2000.tif")
```
