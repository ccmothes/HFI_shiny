...
writeRaster(rast(aligned_rasters), "data/cleaned_rasters.tif", overwrite = TRUE)
```
...
### Proportion time series per country
...
cropped <- terra::crop(rasters_test, countries_test[1])

extracted <- terra::extract(cropped, countries_test[1])

# Get column names of annual raster values
value_cols <- names(extracted)[-1]

#Calculate proportions
final_df <- map(value_cols, function(x) {
  props <- aggregate(
    extracted[x],
  ...
  result_df <- as.data.frame(props[[2]]) %>% pivot_longer(
    cols = c(low, medium, high),
    names_to = "class",
    values_to = x
  ) %>%
    mutate(name = countries[1]$name)
  
  return(result_df)
})

final_df <- reduce(final_df, full_join, by = c("name", "class")) %>% 
  select(name, class, everything())
...
```{r}
#use pbapply for progress tracking
library(pbapply)

process_country <- function(i, rasters, countries) {
  # Crop and extract for single country
  cropped <- terra::crop(rasters, countries[i])
  extracted <- terra::extract(cropped, countries[i])
  
  # Get column names of annual raster values
  value_cols <- names(extracted)[-1]
  
  # Calculate proportions for this country
  country_results <- map(value_cols, function(x) {
    props <- aggregate(
      extracted[x],
      by = list(ID = extracted$ID),
      FUN = function(x) {
    ...
    result_df <- as.data.frame(props[[2]]) %>% 
      pivot_longer(
        cols = c(low, medium, high),
        names_to = "class",
        values_to = x
      ) %>%
      mutate(name = countries[i]$name)
    
    return(result_df)
  })
  
  # Combine results for this country
  country_final <- reduce(country_results, full_join, by = c("name", "class"))
  return(country_final)
}

# Apply to all countries
n_countries <- nrow(countries_test)
all_results <- map_dfr(1:n_countries, ~process_country(.x, rasters_test, countries_test))

all_results <- pblapply(1:n_countries, function(i) {
  process_country(i, rasters_test, countries_test)
})
```
...
all_results <- pblapply(1:n_countries, function(i) {
  process_country(i, rasters, countries)
})
```

```{r}
names(aligned_rasters) <- annual_names
...
Clean for app and save

```{r}
# convert classes to chart labels
annual_props <- results %>% 
